...
const texturePlane = new THREE.TextureLoader().load("@/plane.jpg");
...
function setWalls() {
  ...
  const textureWall = new THREE.TextureLoader().load("@/wall.jpeg");
      ...
      i % 2
        ? // ? new THREE.MeshBasicMaterial({ color: Math.random() * 0xffffff })
          new THREE.MeshBasicMaterial({
            map: new THREE.TextureLoader().load("@/jocondedab.png"),
          })
        : materialWall
    );
